...
//           wrong code Set<string> thisOption = permutationsRec(remaining, chosen ++ ch);
            /*// 错误分析 Originally WRONG CODE was : (,chosen += ch) ,
            // pass para@ chosen + ch to next recursion , but in this level of recursion , the chosen keeps unchanged
//            by the way of caling (, chosen +ch ) , in the wrong code, although the parameter passed by chosen +=ch to next
//            recursion was the same , but the problem arises in current level as chosen was unintendly updated by + ch with
(,chosen += ch)*/
            result += thisOption;
        }
