...
//    (void) numSeats;
//    return {};
    if (populations.size()> numSeats) error("wrong");
    Map<string,int> result;

    for (string country:populations) {
        result[country]=1;
    }
    numSeats -= populations.size();
    HeapPQueue pq;
    for (string country:populations) {
        DataPoint toInsert = {country,(-1.0/sqrt(2))*populations[country]};
        pq.enqueue(toInsert); // DataPoint one = { "First",  1.6108 };
    }

    while(numSeats >0) {
        numSeats--;
        DataPoint biggest_state = pq.dequeue();
        result[biggest_state.name]++;
        float s = 1.0*result[biggest_state.name];

        float p = 1.0*populations[biggest_state.name];
        biggest_state.weight = -1.0*p/sqrt(s*(1 + s));
        pq.enqueue(biggest_state);
