...
HeapPQueue::HeapPQueue() {
    /* TODO: Implement this. */
    elems = new DataPoint[INITIAL_SIZE];   // elems is a pointer from private members of this class
    logicalSize = 0;
...
void HeapPQueue::enqueue(const DataPoint& data) {
    ...
    //once the allocatedSize is out of space; double up its space
    if(logicalSize==allocatedSize-1) /*error("run out of space!")*/ {
        DataPoint *oldelems = elems;  // create a pointer poiting to the old array
        allocatedSize *= 2;
        elems = new DataPoint[allocatedSize]; // new a new array with allocateSize doubled
        for (int i = 1; i <=logicalSize;i++ ) { // from index = 1to logicalsize moving the Datapoint to new array
            elems[i] = oldelems[i];
        }
        delete[] oldelems;// set oldelems free
    }
    logicalSize++;
    elems[logicalSize] = data;
    int index = logicalSize;
    while(index > 1){   // bubble up
        if(elems[index/2].weight > elems[index].weight) {
...
DataPoint HeapPQueue::peek() const {
    /* TODO: Delete the next line and implement this. */
    if (logicalSize==0) error("can not peek a empty heap");
    return elems[1];
}

DataPoint HeapPQueue::dequeue() {   //  there is a bug in dequeue!!
//    /* TODO: Delete the next line and implement this. */
//    if (logicalSize==0) error("can not dequeue a empty heap");

//    swap(elems[1],elems[logicalSize]);
//    logicalSize--;
//    int index = 1;
//    while (index*2 <= logicalSize) {   // it has at least one child
//        if (index*2 + 1 > logicalSize){ // when it only has one child
...
//            }
//        }
//        else {   // when it  has two child
//            int min_child = (elems[index*2].weight < elems[index*2+1].weight)?index*2:index*2+1;
//            if(elems[index].weight >elems[min_child].weight) {
//                swap(elems[min_child],elems[index]);
//                index = min_child;
//            }
            //else break;
...
//    return elems[logicalSize+1];
    if (logicalSize==0) error("can not dequeue empty heap!");
    swap(elems[1],elems[logicalSize]);
    logicalSize--;

    // bubble down
    int index = 1;
    while(2*index <= logicalSize) {
        int cur =2*index;int right=2*index+1;
        if(right<=logicalSize&&elems[right].weight<elems[cur].weight) cur=right;
        if(elems[index].weight>elems[cur].weight) {
            swap(elems[index],elems[cur]);
            index =cur;
        }
        else break;
    }
...
PROVIDED_TEST("Enqueues ten elements in sorted order.") {
    HeapPQueue pq;
    ...
    DataPoint six   = { "Six",    6 };
    DataPoint seven = { "Sseven", 7 };
    DataPoint eight = { "Eight",  8 };
    DataPoint nine  = { "Nine",   9 };
    DataPoint ten  = { "Ten",   10 };
    ...
    /* We should have these contents:
     *
     * [ unused ] [ one ] [ two ] [ three ] [ four ] [ five ]...[ ten ]
     *
    ...
    EXPECT_EQUAL(pq.elems[7], seven);
    EXPECT_EQUAL(pq.elems[8], eight);
    EXPECT_EQUAL(pq.elems[9], nine);
    EXPECT_EQUAL(pq.elems[10], ten);

    /* We are exactly at capacity. No growth should have
     * occurred here.
     */
    EXPECT_EQUAL(pq.allocatedSize, HeapPQueue::INITIAL_SIZE*2);
}
