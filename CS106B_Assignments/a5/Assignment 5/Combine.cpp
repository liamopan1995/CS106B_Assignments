...
#include "Combine.h"
using namespace std;
Vector<DataPoint> merge(const Vector<Vector<DataPoint>>& sequences);
Vector<DataPoint> combine(const Vector<Vector<DataPoint>>& sequences) {
    /* is it time to merge? less than two vector left in sequences*/
    if (sequences.size()<=2) {
        return merge(sequences);
    }
    /**/

    /*split the Vector<Vector<DataPoint>>*/
    int half = sequences.size()/2;
    auto firstHalf = sequences.subList(0,half);
    auto lastHalf = sequences.subList(half);
    /**/

    /*do the combine to each of the halves*/
    /**/
    auto firstcombined=combine(firstHalf);
    auto lastcombined=combine(lastHalf);
    //  Vector<Vector<DataPoint>> continuecombine = {firstcombined,lastcombined};
    /*assemble them into a new sequnces and combine then*/
    return combine({firstcombined,lastcombined});
...
Vector<DataPoint> merge(const Vector<Vector<DataPoint>>& sequences) {
    ...
    /*two elements case  , need to combine them*/
    else{
        Vector<DataPoint> result;
        /*combine the two vectors in descending order*/
        int n1 = sequences[0].size();
        int n2 = sequences[1].size();
        int p1 = 0;
        int p2 = 0;
        while (p1 < n1 && p2 < n2) {
           if (sequences[0][p1].weight <= sequences[1][p2].weight) {
              result.add(sequences[0][p1++]);
           } else {
              result.add(sequences[1][p2++]);
           }
        }
        while (p1 < n1) result.add(sequences[0][p1++]);
        while (p2 < n2) result.add(sequences[1][p2++]);
        return result;
     }
...
PROVIDED_TEST("Preserves duplicates in input sequences.") {  /*  a[i]<=b[i]*/
    Vector<DataPoint> one = {
        { "A" , 1 },
        { "B" , 3 },
    };
