...
LinearProbingHashTable::LinearProbingHashTable(HashFunction<string> hashFn) {
    ...
    for (int i=0;i<nSlots;i++){
        elems[i].type = SlotType::EMPTY;  //I forgot Slottype::
    }
    nelems = 0;
}
...
bool LinearProbingHashTable::insert(const string& elem) {
    ...
    if(nelems==nSlots) return false;
    if (this->contains(elem))return false;
    int hash_place = hsh(elem)%nSlots;
    int actual_place = hash_place;
    do{
        if (elems[actual_place%nSlots].type==SlotType::FILLED) {
            actual_place++;
        }
        else {
            elems[actual_place%nSlots].value=elem;
            elems[actual_place%nSlots].type=SlotType::FILLED;
            this->nelems++;
            return true;
        }
...
bool LinearProbingHashTable::contains(const string& elem) const {
    int hash_place = this->hsh(elem)%nSlots;
    int place = hash_place;
    do {

        if (elems[place%nSlots].type == SlotType::FILLED){
            if (elems[place%nSlots].value==elem) return true;
            place++;
        }
        else {
            place++;
        }
    } while (elems[place%nSlots].type!=SlotType::EMPTY && place%nSlots !=hash_place );
    return false;
}
...
bool LinearProbingHashTable::remove(const string& elem) {
    ...
    int place = hash_place;
    do {
        if (elems[place%nSlots].type == SlotType::FILLED && elems[place%nSlots].value==elem){
            elems[place%nSlots].type=SlotType::TOMBSTONE;
            this->nelems--;
            return true;
        }
        place++;    // a reversion of undeflying code , this makes it cleaner and easy to read
        } while (elems[place%nSlots].type!=SlotType::EMPTY && place%nSlots !=hash_place );

    return false;
}
//        if (elems[place%nSlots].type == SlotType::FILLED){
//            if (elems[place%nSlots].value==elem) {
//                elems[place%nSlots].type=SlotType::TOMBSTONE;
//                this->nelems--;
//                return true;
//            }
...
//        }
//        else {
//            place++;    // 仅当两个if条件同时满足才执行确切语句,剩下的判断都是执行 place++ , 可以把两个if 用与连起来,
                            //然后 进入不了if block的 就执行place++, 这样修改后的代码就是上面那样的,更简洁的
//        }
...
PROVIDED_TEST("Wraps around the end of the table.") {
    ...
    for (string elem: toAdd) {
        EXPECT(table.contains(elem));   // this expect the content inside the
        //brackets to be true
    }
