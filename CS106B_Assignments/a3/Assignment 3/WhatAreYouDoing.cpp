...
Set<string> allEmphasesOf(const string& sentence) {
    ...
    Set<string> result;
    Vector<string> vectorSentence = tokenize(sentence);
    if(vectorSentence.isEmpty()) {
        result += "";
    }
    else {
        string rest = "";
        for (string vec:vectorSentence.subList(1)) {
            rest += vec;}
        Set<string> restSet = allEmphasesOf( rest );
        if (isalpha(vectorSentence[0][0])) {

            for (string s : restSet) {
            result += toLowerCase(vectorSentence[0])  + s ;
            result += toUpperCase(vectorSentence[0])  + s;
            }
        }
        else {
            for (string s : restSet) {
            result += vectorSentence[0] + s;
            }
        }
    }
    return result;
...
//STUDENT_TEST("Enumerates all options in a more typical case.") {
//    Set<string> expected = {
