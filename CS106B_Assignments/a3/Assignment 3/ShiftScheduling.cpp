...
// Function Prototype
Set<Shift> Helper(const Set<Shift>& Left,const Set<Shift>& soFar, int maxHours);


////highestValueScjeduleFor
Set<Shift> highestValueScheduleFor(const Set<Shift>& shifts, int maxHours) {
    if (maxHours < 0) error("!!");
    if (shifts.isEmpty() ) return {};
    Set<Shift> bestSchedule = {};
    return Helper(shifts,bestSchedule,maxHours);
...
Set<Shift> Helper(const Set<Shift>& shifts,const Set<Shift>& soFar, int maxHours) {
    ...
    if (shifts.isEmpty()) {
        ...
        NewShifts -= elem;
        for (auto work:shifts) {
            if(lengthOf(work) > maxHours) { token = 0;break;}
            for (auto job:soFar) {
                if (overlapsWith(job,work)) {
                    token = 0;
                    break; // interfer with the tasks already chosen?
                }
            }
        }
            if(token){

                Set<Shift> pickUp = soFar + elem;
                Set<Shift> a = Helper(NewShifts,pickUp,maxHours - lengthOf(elem));
                Set<Shift> b = Helper(NewShifts,soFar,maxHours);
                //比较两者效用
                int suma = 0;
                int sumb = 0;
                for (Shift item:a) {suma += valueOf(item);}
                for (Shift item:b) {sumb += valueOf(item);}
                if ( suma > sumb) {                     // 枚举 + backtracking 的思想
                    return a;
                }
                else {
                    return b;
                ...
                return Helper(NewShifts,soFar,maxHours );
            }
...
//Set<Shift> HVSFhelp(const Set<Shift>& shifts , const Set<Shift>& sofar,int maxHours)
//{
//    if(shifts.size()==0)
//    {
//        Set<Shift> ans = {};
...
//    {
//        Shift elem = shifts.first();
//        Set<Shift> remaining = shifts-elem; Set<Shift> ifchoose = sofar+elem;

//        double sum = 0;bool flag = 1;
//        for(auto i:ifchoose){sum+=lengthOf(i);}
//        if(sum>maxHours){flag = 0;}//检查是否超时

//        for(auto i : sofar)//检查是否有重叠的班次
//        {
//            if(overlapsWith(i,elem)){flag = 0; break; }
//        }
///*Writter:(zhihu)凉宫春日*/
//        if(flag)
//          {
//              auto x =HVSFhelp(remaining,ifchoose,maxHours);
//              auto y =HVSFhelp(remaining,sofar,maxHours);//都要考虑,可以加也可以不加
//              int sum1 = 0 , sum2 = 0;
//              for(auto p: x)sum1 +=valueOf(p);for(auto q: y)sum2 +=valueOf(q);
//              if(sum1>=sum2)return x;
//              return y;//两条路走完比较价值的大小之后选择一个就好
//          }
//        else
//             return HVSFhelp(remaining,sofar,maxHours);//有就绝对不可以加入这个班次，进入下一个
...
//    else
//    {
//        Set<Shift>ans = HVSFhelp(shifts,{},maxHours);
//        return ans;
//    }
//}
