...
bool isPathToFreedom(MazeCell* start, const string& moves) {
    string moves_ = moves;
    Set<Item> collected={};// initial item list(set)
    collected.add(start->whatsHere);

    while(moves_ !=""){
        //get direct
        char direct = moves_[0];
        switch(direct)
        {
            case 'N':
                if (start->north !=nullptr){

                    start = start->north;// move to there
                    collected.add(start->whatsHere);// collect item here
                    moves_ = moves_.substr(1);// update route
                    break;
                } else return false;
            case 'E':
                if (start->east !=nullptr){

                    start = start->east;// move to there
                    collected.add(start->whatsHere);// collect item here
                    moves_ = moves_.substr(1);// update route
                    break;
                } else return false;
            case 'S':
                if (start->south !=nullptr){
                    ...
                    start = start->south;// move to there
                    collected.add(start->whatsHere);// collect item here
                    moves_ = moves_.substr(1);// update route
                    break;
                } else return false;
            case 'W':
                if (start->west !=nullptr){
                    ...
                    start = start->west;// move to there
                    collected.add(start->whatsHere);// collect item here
                    moves_ = moves_.substr(1);// update route
                    break;
                } else return false;
    ...
    // route is legal , check items are collected
    if (    collected.contains(Item::SPELLBOOK)
            && collected.contains(Item::POTION)
            && collected.contains(Item::WAND)   ) return true;
    else return false;
}
