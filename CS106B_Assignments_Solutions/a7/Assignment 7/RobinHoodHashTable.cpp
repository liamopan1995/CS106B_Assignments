...
RobinHoodHashTable::RobinHoodHashTable(HashFunction<string> hashFn) {
    ...
    myhash = hashFn;
    nSlots = hashFn.numSlots();
    elems = new Slot[nSlots];  //  class private zhong dingyi de yige zhizhen mingzi jiushi elems
...
bool RobinHoodHashTable::insert(const string& elem) {
    std:: string elem_ =elem;
    if (contains(elem_)) return false;
    int hash_place = this->myhash(elem_)%nSlots;
    int actual_place = hash_place;
    int steps = 0;
    do {

        if (elems[actual_place%nSlots].distance==EMPTY_SLOT) {
            elems[actual_place%nSlots].value = elem_;
            elems[actual_place%nSlots].distance = steps;
            this->nelems++;
            return true;
        }
        else if (elems[actual_place%nSlots].distance < steps) {

            swap(elems[actual_place%nSlots].distance,steps);
            swap(elems[actual_place%nSlots].value,elem_);
        }
        steps++;
        actual_place++;

    } while (actual_place%nSlots !=hash_place);
    return false;
}

bool RobinHoodHashTable::contains(const string& elem) const {
    int hash_place = this->myhash(elem)%nSlots;
    int actual_place = hash_place;
    int steps = 0;
        do {int idx_slot = actual_place%nSlots;
        if (elems[idx_slot].distance!=EMPTY_SLOT && elems[idx_slot].value==elem) return true;
         if (elems[idx_slot].distance==EMPTY_SLOT || elems[actual_place%nSlots].distance <steps ) return false;
         steps++;
         actual_place++;
    } while ( actual_place%nSlots !=hash_place /*|| elems[actual_place%nSlots].distance >steps*/);// 和上面的条件互斥的,写了拖慢程序速度
    return false;
}
...
bool RobinHoodHashTable::remove(const string& elem) {
    ...
    int steps = 0;
    do {
        int idx_slot = actual_place%nSlots;
        if (elems[idx_slot].distance!=EMPTY_SLOT && elems[idx_slot].value==elem) {
            nelems--;
            elems[idx_slot].distance = EMPTY_SLOT;
            while(elems[(actual_place+1) % nSlots].distance!= EMPTY_SLOT
                  && elems[(actual_place+1) % nSlots].distance!= 0)
            {
                swap(elems[actual_place % nSlots].value, elems[(actual_place +1 )% nSlots].value);
                swap(elems[actual_place % nSlots].distance, elems[(actual_place +1 )% nSlots].distance);
                elems[(actual_place)% nSlots].distance--;
                actual_place++;
             }
            return true;
        }
...
void RobinHoodHashTable::printDebugInfo() const {
    ...
    cout<<("num of slots")<<this->nSlots<<endl;
    cout<<("num of elems")<<this->nelems<<endl;
    for (int i=0;i<nSlots;i++) {
        cout<<elems[i].value<<endl;
        cout<<elems[i].distance<<endl;
    }
}
