...
void deleteNucleotides(Nucleotide* dna) {
    ...
    while (dna != nullptr) {
        struct Nucleotide * temp = dna;
        dna = dna->next;
        delete temp;  //  difference btween
...
string fromDNA(Nucleotide* dna) {
    ...
    string result= "";
    if(dna == nullptr) return result;
    while(true){             // while(true){...;   if (dna->next==nullptr) break; dna = dna->next; }经典链表框架
...
Nucleotide* toStrand(const string& str) {
    ...
    Nucleotide* dna = result;
    dna -> prev = nullptr;
    dna -> next = nullptr;

 // build first letter wich is of ACGT  ,then create the cell
    while(true){

        if(str[start]== 'A' || str[start]== 'C'|| str[start]== 'G'|| str[start]== 'T'){
            dna -> value = str[start];
            start++;
            break;
...
// continue building strand from the place where found first letter of acgt
...
Nucleotide* findFirst(Nucleotide* dna, Nucleotide* target) {
    /* TODO: Delete this comment and the next lines and implement this function. */
    if (dna == nullptr) return  dna;
    if (target == nullptr) return  dna;
    Nucleotide* target_start = target;
//    Nucleotide* dna_ = dna;

    Nucleotide* left = nullptr;
    while(true){
        while(left == nullptr ) {  // find "head" of target in dna
            if ( dna->value==target->value ){
                left = dna;
                if (target -> next == nullptr) return left;
                if(/*target->next !=nullptr && */ dna->next==nullptr) return nullptr;
                target = target -> next;
                dna = dna -> next;
                break;
            }
                ...
                if(target->next != nullptr && dna->next==nullptr) return nullptr;
                if (dna->next != nullptr)  dna = dna ->next;
                }

        }
        // do sth to find the rest of target_}
        // if this attempt failed : turn over. left = nullptr ;
        while (left != nullptr){
            if (target->value == dna->value) {
                ...
                if( dna->next == nullptr ) return nullptr;
                dna = left ->next;
                left = nullptr;
                target = target_start;
...
bool spliceFirst(Nucleotide*& dna, Nucleotide* target) {
    /* TODO: Delete this comment and the next lines and implement this function. */
    if (dna == nullptr) return true;
    if (target == nullptr) return true;

    Nucleotide*  trim_start = findFirst(dna,target);
    if (trim_start==nullptr) return false;

    Nucleotide*  trim_iter = trim_start;
    while (target->next != nullptr) {
        trim_iter = trim_iter->next;
        target = target->next;
    }



    // // ++****++ , original dna ++.. to tirm: *..
    if (trim_start->prev != nullptr && trim_iter->next != nullptr) {
        Nucleotide* trim_1_half = trim_start->prev;
        Nucleotide* trim_2_half = trim_iter->next;

        trim_iter->next = nullptr;
        trim_1_half->next = trim_2_half;
        trim_2_half->prev = trim_1_half;
        deleteNucleotides(trim_start);//
        return true;
    }
    else { // ++**** , to tirm: *
        if (trim_iter->next == nullptr && trim_start->prev !=nullptr) {
            Nucleotide* trim_1_half = trim_start->prev;
            trim_1_half->next = nullptr;
            deleteNucleotides(trim_start);//
            return true;
        // ***++ here s bug fixed
        } else if (trim_start->prev == nullptr && trim_iter->next != nullptr)
        {
            dna = trim_iter->next;  // dna is of Necleotide* type , let dna point to the cell after trim_iter
            // do not confuse it with : dna->next = trim_iter->next ,this only modifys which cell the dna's
            // next pointer points to. but dna will still point to the old cell
            dna -> prev = nullptr;
            trim_iter ->next = nullptr;
            deleteNucleotides(trim_start);//
...
PROVIDED_TEST("toStrand does not convert characters which are not ACGTin strings  to a strand. ") {
    Nucleotide* dna = toStrand("A");
    EXPECT(dna != nullptr);
    EXPECT_EQUAL(dna->value, 'A');
    EXPECT_EQUAL(dna->next, nullptr);
...
STUDENT_TEST("fromDNA handles letter that are not of ACGT.") {
    Nucleotide* one   = new Nucleotide;
    Nucleotide* two   = new Nucleotide;
    Nucleotide* three = new Nucleotide;
    Nucleotide* four  = new Nucleotide;
    ...
    one->value   = 'A';
    two->value   = 'X'; // changed this to 'X'
    three->value = 'T';
    four->value  = 'G';

    EXPECT_EQUAL(fromDNA(one), "ATG");
    deleteNucleotides(one);
}
...
STUDENT_TEST("deleteNucleotides cleans up a  sequence ring.") {
    Nucleotide* dna = new Nucleotide;
    dna->value = 'A';
    dna->prev = nullptr;
    ...
    dna->next->value = 'C';
    dna->next->prev = dna;
    dna->next->next = nullptr;// from nullptr changed to dna
...
// 打印 链表环  // this will run forever
//PROVIDED_TEST("fromDNA handles strand in form of a ring.") {
//    Nucleotide* one   = new Nucleotide;
//    Nucleotide* two   = new Nucleotide;
//    Nucleotide* three = new Nucleotide;
//    Nucleotide* four  = new Nucleotide;
//    // a ring : one two three four one..
//    one->prev    = nullptr;
//    one->next    = two;
...
//    four->prev   = three;
//    four->next   = one;  //  change this to one
...
STUDENT_TEST("toStrand does not convert characters which are not ACGTin strings  to a strand. ") {
    Nucleotide* dna = toStrand("BBABTGTB");
    EXPECT(dna != nullptr);
    EXPECT_EQUAL(fromDNA(dna), "ATGT");
    deleteNucleotides(dna);
}
