...
Map<string, double> kGramsIn(const string& str, int kGramLength) {
    ...
    for(int i = 0;i<(length - kGramLength + 1);i++) {

    kGramsMap[str.substr(i,kGramLength )]++;
    }
    return kGramsMap;
}
...
Map<string, double> normalize(const Map<string, double>& input) {
    /* Notes :
     * A.for (variable type key_name :collection_name)   不用书上的foreach，那个没研究
     * 明白
     * B. 不知道 明明有 const & 标示， 为什么还要复制一个新的map 在新的map上normalize数据后返回
     * 新的map， 直接操作传参来的不被允许。
     *
     */

    double sum = 0;  // sum of frequncies

    for (string s: input) {
        sum += input[s] * input[s];

    }
    if (sum == 0) error("current kGram contains no meaningful frequency ");
    Map<string,double> ans=input;
    for(string s: ans) {
        ans[s] = ans[s] / sqrt(sum);
    }
    return ans;
//    for(string s: input)
//    {
//        input[s] = input[s]/sqrt(sum);
//    }
//    return input;
...
Map<string, double> topKGramsIn(const Map<string, double>& source, int numToKeep) {
     ...
     * this function.
     * Notice:
     * A. is negative wight by priority queue allowed, if it is, how does it behave?
     */

    PriorityQueue<string> pq;
    Map<string, double> sortedList = source;
    for (string key:source) {
        pq.enqueue(key, source[key]);
    }
    int sizeOfSource = source.size();
    for (int i = 0;i < sizeOfSource - numToKeep;i++) {
    sortedList.remove(pq.dequeue());
    }
    return sortedList;
...
double cosineSimilarityOf(const Map<string, double>& lhs, const Map<string, double>& rhs) {
    ...
    for (string key: lhs) {
        if (rhs.containsKey( key )) {
            cosineSimilarity += lhs[key] * rhs[key];
...
string guessLanguageOf(const Map<string, double>& textProfile,
    ...
    /* TODO: Delete this comment and the other lines here, then implement
     * this function.
     *  A. can't traves Set by for int i... , because Set is unordered
     */

    double bestCosineSimilarity = 0;
    string bestGuess = "";
    if(corpora.isEmpty()) {
        error("the given corpora is empty!");
        return "";}
    for (Corpus corpus: corpora) {                      // this is how to traves over a Set
        if (cosineSimilarityOf(textProfile, corpus.profile) > bestCosineSimilarity ) {
            bestCosineSimilarity = cosineSimilarityOf(textProfile,corpus.profile);  // careful with
            //how you refer to a member of struct :(corpus.profile)
            bestGuess = corpus.name;
        }
