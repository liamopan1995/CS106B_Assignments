...
string aSequenceOfOrder(int n) {
    ...
    if (n == 0) {
        ...
        result += aSequenceOfOrder( n - 1 ) + bSequenceOfOrder( n - 1 );
    }
    return result;
}
...
string bSequenceOfOrder(int n) {
    ...
    if (n == 0) {
        ...
        result +=bSequenceOfOrder( n - 1 ) + aSequenceOfOrder( n - 1 );
    }
    return result;
}
...
STUDENT_TEST("No bad behaviors on input begin with 0 .") {
    /* input contains 0 at the begining won't disturb the result */
    EXPECT_EQUAL(aSequenceOfOrder(003), "ABBABAAB");
    EXPECT_EQUAL(bSequenceOfOrder(0003), "BAABABBA");
}
